...
func TestServices_Asset(t *testing.T) {
	...
	t.Log("Create asset")
	assetID, err := assetService.Create(context.Background(), asset)
	require.NoError(t, err)

	t.Log("Get asset")
	rasset, err := assetService.Get(context.Background(), userID, assetID)
	require.NoError(t, err)
	...
	// rasset, err = db.GetAsset(context.Background(), 1, id)
